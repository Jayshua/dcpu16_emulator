...
#[test]
fn program_crc_b() {
   // This program is the same as above, but on a different string
   // At the end of execution, z should be 0x12b6
   let mut dcpu = Dcpu::new();
   ...
   dcpu.memory[87] = 0x0054;
   dcpu.memory[88] = 0x0020;
   dcpu.memory[89] = 0x0054;
   dcpu.memory[90] = 0x0065;
   dcpu.memory[91] = 0x0073;
   dcpu.memory[92] = 0x0074;
   dcpu.memory[93] = 0x0020;
   dcpu.memory[94] = 0x0074;
   dcpu.memory[95] = 0x0073;
   dcpu.memory[96] = 0x0065;
   ...
   println!("Steps: {}", dcpu.cycle_count);
   assert_eq!(dcpu.registers[5], 0x12b6);
}